...
export async function bootRoom(canvasId='scene', roomName='palace_throne') {
  ...
  try {
    ...
    for (const p of list) {
      if (p.type === 'light_point') {
        const col = new THREE.Color(p.color || '#ffffff');
        const light = new THREE.PointLight(col, p.intensity ?? 1.0, p.distance ?? 8, p.decay ?? 2.0);
        light.position.set(...(p.pos || [0,2,0]));
        scene.add(light);
        if (p.flicker) {
          flickers.push({ light, base: p.intensity ?? 1.0, t: Math.random()*100 });
        }
        continue;
      }
      // mesh prop
  ...
  function animate() {
    controls.update();
    // Candle/cauldron flicker
    const t = performance.now() * 0.001;
    for (const f of flickers) {
      f.light.intensity = f.base * (0.85 + 0.15*Math.sin(t*7.3 + f.t) + 0.05*Math.sin(t*13.1 + f.t*0.5));
    }
